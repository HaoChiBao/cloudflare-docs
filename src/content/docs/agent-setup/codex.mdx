...
<Tabs urlParam="view">
	<TabItem label="Terminal" value="terminal">
		<Steps>
			1. **Install Codex**
				...
				For Homebrew or other install methods, see the [Codex CLI setup guide](https://developers.openai.com/codex/cli/).
	...
	<TabItem label="ChatGPT desktop app" value="desktop">
		<Steps>
...
- The Cloudflare API MCP server uses Code Mode — Codex writes JavaScript against a typed API to reach any of 2,500+ endpoints and current developer documentation in approximately 1,100 tokens.
...
<TroubleshootingList>
	...
	<TroubleshootingItem issue="Getting outdated information about Cloudflare products">
		Enable the <a href="https://github.com/cloudflare/mcp" target="_blank" rel="noopener noreferrer">Cloudflare API MCP server</a> so the agent can fetch
