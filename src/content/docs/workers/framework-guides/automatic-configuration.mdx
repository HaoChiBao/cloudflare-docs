...
For Next.js projects, automatic configuration sets up [R2](/r2/) for caching if your Cloudflare account has R2 enabled. R2 caching improves performance for [Incremental Static Regeneration (ISR)](https://opennext.js.org/cloudflare/caching) and other Next.js caching features.

If R2 is enabled on your account, automatic configuration creates an R2 bucket and configures caching. If R2 is not enabled, automatic configuration configures your project without caching. You can [enable R2](/r2/get-started/) later and manually configure [OpenNext caching](https://opennext.js.org/cloudflare/caching).
