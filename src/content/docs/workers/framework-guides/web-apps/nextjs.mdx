...
Cloudflare recommends [vinext](https://vinext.dev/) as the default way to run Next.js applications on Cloudflare Workers. Scaffold a new Workers-ready app with `create-vinext-app`, or add vinext to an existing Next.js 16 app with `vinext init`. The initializer is non-destructive, so your existing `next dev` command continues to work. Neither path requires a Cloudflare-specific template.

To continue with OpenNext or Next on Pages, refer to [other Next.js deployment paths](#use-another-nextjs-deployment-path).
